...
This container image is used as a [custom runner image](https://docs.spacelift.io/integrations/docker#customizing-the-runner-image) for Spacelift stacks. It is built from the [default runner image](https://github.com/spacelift-io/runner-terraform) and includes additional utilities like the 1Password CLI. It is available on [GHCR](https://github.com/br3ndonland/infrastructure-live/pkgs/container/spacelift-runner-terraform) and [AWS ECR Public](https://gallery.ecr.aws/br3ndonland/spacelift-runner-terraform).

The [1Password CLI](https://developer.1password.com/docs/cli/get-started/) (`op`) was required if using the [1Password OpenTofu provider](https://developer.1password.com/docs/terraform/?workflow-type=1password-cli) prior to version 3. It would be simplest to install `op` in a `before_init` [stack hook](https://docs.spacelift.io/concepts/stack/stack-settings#customizing-workflow), but Spacelift runner images use a non-root user `spacelift` that cannot install apk repositories, so a custom image build is needed. `op` does not support Alpine Linux on `arm64` (`aarch64`). If attempting to install `op` on `linux/arm64` with Alpine Linux and `apk`, the following warning may be seen: "WARNING: updating and opening https://downloads.1password.com/linux/alpinelinux/stable/: No such file or directory". So, a custom runner image was built in order to use `op` with the 1Password provider.

In version 3, 1Password migrated the provider to the 1Password SDK, and it no longer requires the 1Password CLI ([1Password/terraform-provider-onepassword#228](https://github.com/1Password/terraform-provider-onepassword/issues/228), [1Password/terraform-provider-onepassword@v3.0.0](https://github.com/1Password/terraform-provider-onepassword/releases/tag/v3.0.0)). Although the 1Password provider no longer requires the 1Password CLI, a custom runner image is still needed because the 1Password SDK requires additional dependencies on Alpine Linux ([1Password/terraform-provider-onepassword#340](https://github.com/1Password/terraform-provider-onepassword/issues/340)).

AWS [ECR Public](https://docs.aws.amazon.com/AmazonECR/latest/public/public-registries.html) only supports `us-east-1` (see the [`ecrpublic_repository` resource](https://search.opentofu.org/provider/hashicorp/aws/latest/docs/resources/ecrpublic_repository) and [hashicorp/terraform-provider-aws#18047](https://github.com/hashicorp/terraform-provider-aws/issues/18047)). The ECR Public registry for each account is assigned a default alias that does not match the account alias. A custom alias can be requested, but it must be approved by AWS. This repo publishes to the `br3ndonland` alias.
...
3. Provision AWS IAM roles for Spacelift.
   - The roles are provisioned using [a stack in this repo](./stacks/aws/br3ndonland/spacelift/main.tf). This presents a "[chicken-and-egg](https://en.wikipedia.org/wiki/Chicken_or_the_egg)" problem - the configurations will provision roles for all stacks to use, but the roles will not exist until the configurations are applied - so how do these configurations themselves get credentials? One solution is to apply the configurations locally or with static AWS access key credentials the first time to provision the roles that will be used in later applies.
...
4. Use the AWS IAM roles to provision [integrations](https://docs.spacelift.io/integrations/cloud-providers) for each [Spacelift space](https://docs.spacelift.io/concepts/spaces).
   ...
   - Integrations are an awkward abstraction - they can be _set_ on spaces, but they can also be separately _attached_ to stacks.
   - The standard way to set up an AWS integration on Spacelift is to allow role assumption by `sts:ExternalId`. The [docs on programmatic setup of the standard AWS integration](https://docs.spacelift.io/integrations/cloud-providers/aws#programmatic-setup) present a chicken-and-egg problem (actually more like a circular dependency) between `spacelift_aws_integration` and the role used in the integration by using `spacelift_aws_integration_attachment_external_id` data sources. This is not recommended because the data sources are only needed for the `assume_role_policy_statement` attribute, but this can be set up directly without a data source. A better approach is to set up the roles first with trust policies like the ones in the [getting started docs](https://docs.spacelift.io/getting-started/integrate-cloud/AWS), do a `tofu apply` with local credentials, then use the role ARNs (or read them with `terraform_remote_state`) when setting up `spacelift_aws_integration` resources.
   - [OIDC integrations](https://docs.spacelift.io/integrations/cloud-providers/oidc) alleviate some of the awkwardness because they don't have to be attached to specific stacks - OIDC subject claims can specify spaces and stacks for greater specificity. OIDC integrations have some notable limitations. **Spacelift only offers OIDC on paid plans**. By comparison, HashiCorp allows [Terraform Cloud OIDC](https://developer.hashicorp.com/terraform/cloud-docs/dynamic-provider-credentials) on free plans (though this may change after the [free tier changes in 2026](https://www.hashicorp.com/en/blog/continuing-hcp-terraform-s-enhanced-free-tier-experience)). Another limitation is that [there is only limited support for custom OIDC subject claims](https://feedback.spacelift.io/p/allow-adding-space-hierarchy-in-oidc-subject). Note that `space:` claims must reference the space _id_, not the space name. The OIDC claims in this repo are customized to create dedicated OIDC roles for each space. The AWS space has broad privileges needed to manage AWS resources. The GitHub and Spacelift spaces have only the least privilege needed to read state for the S3 backend.
   - Note an important difference between standard AWS integrations and OIDC AWS integrations. **Spacelift standard AWS integrations do not require `assume_role` blocks in provider or backend configurations because they export credentials as environment variables (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_SECURITY_TOKEN`, `AWS_SESSION_TOKEN`. Spacelift OIDC AWS integrations do require `assume_role_with_web_identity` blocks in S3 backend and AWS provider configurations.**
...
6. Build [custom runner images](https://docs.spacelift.io/integrations/docker#customizing-the-runner-image). See the [containers section](#containers) for more details.
